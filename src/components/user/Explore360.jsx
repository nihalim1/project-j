...
export default function Explore360() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const q = query(collection(db, "content_explore"), orderBy("title"));
        const snap = await getDocs(q);
        const list = [];
        snap.forEach((d) => list.push({ id: d.id, ...d.data() }));
        setItems(list);
      } catch (error) {
        console.error("fetch explore content error", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <section className="space-y-3">
      <div>
        <h3 className="text-lg font-semibold text-slate-800">{t("user.explore.title")}</h3>
        <p className="text-sm text-slate-600">{t("user.explore.subtitle")}</p>
      </div>
      {loading ? (
      ...
      {modalItem && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center px-4 py-8" onClick={() => setModalItem(null)}>
          <div
          ...
          >
            <div className="relative aspect-[4/3] overflow-hidden bg-slate-100">
              ...
              {modalItem.image ? (
                <img src={modalItem.image} alt={currentLanguage === "ms" && modalItem.title_ms ? modalItem.title_ms : modalItem.title} className="w-full h-full object-cover" />
              ) : (
